...
def detect_and_color_splash(model, image_path=None, img_file_name=None, image_dir=None):
...
realList = []
errList = []
def back_crack(model, img, image_dir):
    imgname = img.split("/")[-1]
    onlyname, _ = os.path.splitext(imgname)
    imgname_bmp = onlyname + '.bmp'
    # output_imgname = os.path.join(image_path, imgname_png)
    error, mTime = detect_and_color_splash(model, image_path=img,  img_file_name=imgname_bmp, image_dir=image_dir)
    if error:
