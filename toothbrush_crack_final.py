...
class ToothBrushCrackDataset(utils.Dataset):
    ...
    def load_toothbrush_crack(self, dataset_dir, data_list): #, subset):
        ...
        annotations = json.load(open("../total_via_region_data.json"))
...
def train(model):
    ...
    kf = KFold(n_splits=N_FOLDS, random_state=44, shuffle=True)  # the best is 47
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', required=False,
                        ...
                        default='./models/front_crack/mask_rcnn_toothbrush_crack_0084.h5',  # 'coco',
    ...
    if args.command == "train":
        ...
        for i in range(len(class_df)):
            if class_df.loc[i, 'ground truth'] == class_df.loc[i, 'prediction']:
                if class_df.loc[i, 'ground truth'] == 0: # NORMAL
                    TP += 1
                else: # ERROR
                    TN += 1
